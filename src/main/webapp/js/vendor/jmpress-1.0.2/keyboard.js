...
(function( factory ) {
	...
	$jmpress( "afterInit", function( nil, eventData ) {
		...
		.bind( "keypress" + current.keyboardNamespace, function( event ) {
			var nodeName;
			for ( nodeName in ignoreKeyboardSettings ) {
		...
		.bind( "keydown" + current.keyboardNamespace, function( event ) {
			var nodeName, nextFocus, action,
				reverseSelect = false,
				eventTarget = $( event.target );

			if ( !settings.fullscreen && !eventTarget.closest( jmpress ).length ||
					!keyboardSettings.use ) {
				return;
			}

			for ( nodeName in ignoreKeyboardSettings ) {
				if ( eventTarget[ 0 ].nodeName === nodeName &&
						ignoreKeyboardSettings[ nodeName ].indexOf( event.which ) !== -1 ) {
					return;
				}
			}
			...
			action = keyboardSettings.keys[ event.which ];
